...
#' for benthic and geomorphic habitat classes from the output of [set_roi()],
...
#'   \item{stats}{A named list with data frames `benthic` and `geomorph`.}
#' }
#'
#' @details
#' For each habitat class the function computes area in km2 from pixel size
#' at native resolution (~10 m), percentage cover relative to total classified
#' area within the ROI, and mean, minimum, and maximum depth derived by
#' intersecting the habitat raster with the aligned GEBCO bathymetry layer.
#' Results are presented as a styled gt table printed automatically to the
#' RStudio Viewer, and returned as raw data frames for further analysis.
#'
#' @references
